#import "UTCPTUSBHub.h"
#import "UTCPTPrivate.h"
...
NSString * const UTCPTUSBHubErrorDomain = @"PTUSBHubError";
...
NSString * const UTCPTUSBDeviceDidAttachNotification = @"PTUSBDeviceDidAttachNotification";
NSString * const UTCPTUSBDeviceDidDetachNotification = @"PTUSBDeviceDidDetachNotification";
...
// (device) system. In practice, a UTCPTUSBChannel is connected to a usbmuxd
// endpoint which is configured to either listen for device changes (the
// UTCPTUSBHub's channel is usually configured as a device notification listener) or
// configured as a TCP bridge (e.g. channels returned from UTCPTUSBHub's
// connectToDevice:port:callback:). You should not create channels yourself, but
// let UTCPTUSBHub provide you with already configured channels.
@interface UTCPTUSBChannel : NSObject {
...
// own I/O logic without UTCPTUSBChannel. Remember to dispatch_retain() the channel
// if you plan on using it as it might be released from the UTCPTUSBChannel at any
// point in time.
...
@interface UTCPTUSBChannel (Private)
...
@interface UTCPTUSBHub () {
  UTCPTUSBChannel *channel_;
}
- (void)handleBroadcastPacket:(NSDictionary*)packet;
@end


@implementation UTCPTUSBHub


+ (UTCPTUSBHub*)sharedHub {
  static UTCPTUSBHub *gSharedHub;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    gSharedHub = [UTCPTUSBHub new];
...
- (void)connectToDevice:(NSNumber*)deviceID port:(int)port onStart:(void(^)(NSError*, dispatch_io_t))onStart onEnd:(void(^)(NSError*))onEnd {
  UTCPTUSBChannel *channel = [UTCPTUSBChannel new];
  NSError *error = nil;
  ...
  NSDictionary *packet = [UTCPTUSBChannel packetDictionaryWithPacketType:kPlistPacketTypeConnect
...
- (void)handleBroadcastPacket:(NSDictionary*)packet {
  ...
  if ([@"Attached" isEqualToString:messageType]) {
    [[NSNotificationCenter defaultCenter] postNotificationName:UTCPTUSBDeviceDidAttachNotification object:self userInfo:packet];
  } else if ([@"Detached" isEqualToString:messageType]) {
    [[NSNotificationCenter defaultCenter] postNotificationName:UTCPTUSBDeviceDidDetachNotification object:self userInfo:packet];
  } else {
...
@implementation UTCPTUSBChannel
...
- (void)listenWithBroadcastHandler:(void(^)(NSDictionary *packet))broadcastHandler callback:(void(^)(NSError*))callback {
  ...
  NSDictionary *packet = [UTCPTUSBChannel packetDictionaryWithPacketType:kPlistPacketTypeListen payload:nil];
...
- (BOOL)errorFromPlistResponse:(NSDictionary*)packet error:(NSError**)error {
  if (!*error) {
    ...
    if (!n) {
      *error = [NSError errorWithDomain:UTCPTUSBHubErrorDomain code:(n ? n.integerValue : 0) userInfo:nil];
      return NO;
    }
    ...
    if (replyCode != 0) {
      ...
      *error = [NSError errorWithDomain:UTCPTUSBHubErrorDomain code:replyCode userInfo:[NSDictionary dictionaryWithObject:errmessage forKey:NSLocalizedDescriptionKey]];
      return NO;
    }
  }
  return YES;
...
- (void)scheduleReadPacketWithCallback:(void(^)(NSError*, NSDictionary*, uint32_t))callback {
  ...
  dispatch_io_read(channel_, 0, sizeof(ref_upacket.size), queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    UTCPT_PRECISE_LIFETIME_UNUSED dispatch_data_t map_data = dispatch_data_create_map(data, (const void **)&buffer, &buffer_size); // objc_precise_lifetime guarantees 'map_data' isn't released before memcpy has a chance to do its thing
    ...
    dispatch_io_read(channel_, offset, upacket->size - offset, queue_, ^(bool done, dispatch_data_t data, int error) {
      ...
      if (upacket_len > kUsbmuxPacketMaxPayloadSize) {
        callback(
          [[NSError alloc] initWithDomain:UTCPTUSBHubErrorDomain code:1 userInfo:@{
      ...
      char *buffer = NULL;
      size_t buffer_size = 0;
      UTCPT_PRECISE_LIFETIME_UNUSED dispatch_data_t map_data = dispatch_data_create_map(data, (const void **)&buffer, &buffer_size);
      ...
      if (upacket->protocol != USBMuxPacketProtocolPlist) {
        callback([[NSError alloc] initWithDomain:UTCPTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package protocol" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
      ...
      if (upacket->type != USBMuxPacketTypePlistPayload) {
        callback([[NSError alloc] initWithDomain:UTCPTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package type" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
