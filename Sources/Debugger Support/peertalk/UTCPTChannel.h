...
@class UTCPTData, UTCPTAddress;
@protocol UTCPTChannelDelegate;

@interface UTCPTChannel : NSObject

// Delegate
@property (strong) id<UTCPTChannelDelegate> delegate;

// Communication protocol. Must not be nil.
@property UTCPTProtocol *protocol;
...
// Create a new channel using the shared UTCPTProtocol for the current dispatch
// queue, with *delegate*.
...
@interface UTCPTAddress : NSObject
...
// Protocol for UTCPTChannel delegates
@protocol UTCPTChannelDelegate <NSObject>
...
- (BOOL)ioFrameChannel:(UTCPTChannel*)channel shouldAcceptFrameOfType:(uint32_t)type tag:(uint32_t)tag payloadSize:(uint32_t)payloadSize;
