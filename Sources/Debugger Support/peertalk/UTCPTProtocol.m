...
static const uint32_t UTCPTProtocolVersion1 = 1;

NSString * const UTCPTProtocolErrorDomain = @"PTProtocolError";

// This is what we send as the header for each frame.
typedef struct _UTCPTFrame {
...
static void _release_queue_local_protocol(void *objcobj) {
  if (objcobj) {
    UTCPTProtocol *protocol = (__bridge_transfer id)objcobj;
    protocol->queue_ = NULL;
  }
}


@interface UTCPTRQueueLocalIOFrameProtocol : UTCPTProtocol
@end
@implementation UTCPTRQueueLocalIOFrameProtocol
- (void)setQueue:(dispatch_queue_t)queue {
}
@end


@implementation UTCPTProtocol


+ (UTCPTProtocol*)sharedProtocolForQueue:(dispatch_queue_t)queue {
  static const char currentQueueFrameProtocolKey;
  //dispatch_queue_t queue = dispatch_get_current_queue();
  UTCPTProtocol *currentQueueFrameProtocol = (__bridge UTCPTProtocol*)dispatch_queue_get_specific(queue, &currentQueueFrameProtocolKey);
  if (!currentQueueFrameProtocol) {
    currentQueueFrameProtocol = [[UTCPTRQueueLocalIOFrameProtocol alloc] initWithDispatchQueue:NULL];
    ...
    return (__bridge UTCPTProtocol*)dispatch_queue_get_specific(queue, &currentQueueFrameProtocolKey); // to avoid race conds
  } else {
...
- (dispatch_data_t)createDispatchDataWithFrameOfType:(uint32_t)type frameTag:(uint32_t)frameTag payload:(dispatch_data_t)payload {
  UTCPTFrame *frame = CFAllocatorAllocate(kCFAllocatorDefault, sizeof(UTCPTFrame), 0);
  frame->version = htonl(UTCPTProtocolVersion1);
  ...
  dispatch_data_t frameData = dispatch_data_create((const void*)frame, sizeof(UTCPTFrame), queue_, ^{
...
- (void)readFrameOverChannel:(dispatch_io_t)channel callback:(void(^)(NSError *error, uint32_t frameType, uint32_t frameTag, uint32_t payloadSize))callback {
  ...
  dispatch_io_read(channel, 0, sizeof(UTCPTFrame), queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    if (done) {
      ...
      if (dataSize == 0) {
        callback(nil, UTCPTFrameTypeEndOfStream, 0, 0);
        return;
      }
      
      if (!allData || dispatch_data_get_size(allData) < sizeof(UTCPTFrame)) {
#if UTCPT_DISPATCH_RETAIN_RELEASE
        if (allData) dispatch_release(allData);
#endif
        callback([[NSError alloc] initWithDomain:UTCPTProtocolErrorDomain code:0 userInfo:nil], 0, 0, 0);
        return;
      }
      
      UTCPTFrame *frame = NULL;
      size_t size = 0;
      
      UTCPT_PRECISE_LIFETIME dispatch_data_t contiguousData = dispatch_data_create_map(allData, (const void **)&frame, &size); // precise lifetime guarantees bytes in frame will stay valid till the end of scope
#if UTCPT_DISPATCH_RETAIN_RELEASE
      ...
      if (frame->version != UTCPTProtocolVersion1) {
        callback([[NSError alloc] initWithDomain:UTCPTProtocolErrorDomain code:0 userInfo:nil], 0, 0, 0);
      } else {
...
- (void)readPayloadOfSize:(size_t)payloadSize overChannel:(dispatch_io_t)channel callback:(void(^)(NSError *error, dispatch_data_t contiguousData, const uint8_t *buffer, size_t bufferSize))callback {
  ...
  dispatch_io_read(channel, 0, payloadSize, queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    if (done) {
      ...
      uint8_t *buffer = NULL;
      size_t bufferSize = 0;
      UTCPT_PRECISE_LIFETIME dispatch_data_t contiguousData = NULL;
...
- (void)readFramesOverChannel:(dispatch_io_t)channel onFrame:(void(^)(NSError*, uint32_t, uint32_t, uint32_t, dispatch_block_t))onFrame {
  [self readFrameOverChannel:channel callback:^(NSError *error, uint32_t type, uint32_t tag, uint32_t payloadSize) {
    onFrame(error, type, tag, payloadSize, ^{
      if (type != UTCPTFrameTypeEndOfStream) {
...
@interface _UTCPTDispatchData : NSObject {
  dispatch_data_t dispatchData_;
}
@end
@implementation _UTCPTDispatchData
...
- (dispatch_data_t)utcpt_createReferencingDispatchData {
...
+ (NSData *)utcpt_dataWithContentsOfDispatchData:(dispatch_data_t)data {
  if (!data) {
    return nil;
  }
  uint8_t *buffer = NULL;
  size_t bufferSize = 0;
  UTCPT_PRECISE_LIFETIME dispatch_data_t contiguousData = dispatch_data_create_map(data, (const void **)&buffer, &bufferSize);
  if (!contiguousData) {
    return nil;
  }
  
  _UTCPTDispatchData *dispatchDataRef = [[_UTCPTDispatchData alloc] initWithDispatchData:contiguousData];
...
- (dispatch_data_t)utcpt_createReferencingDispatchData {
  ...
  if (!plistData) {
    ...
    return [plistData utcpt_createReferencingDispatchData];
  }
}

// Decode *data* as a peroperty list-encoded dictionary. Returns nil on failure.
+ (NSDictionary*)utcpt_dictionaryWithContentsOfDispatchData:(dispatch_data_t)data {
  if (!data) {
    return nil;
  }
  uint8_t *buffer = NULL;
