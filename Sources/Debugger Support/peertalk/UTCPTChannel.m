...
#define UTCPT_SOCKADDR_ACCESS(ss, member4, member6) \
...
- (id)initWithProtocol:(UTCPTProtocol*)protocol {
  if (!(self = [super init])) return nil;
  protocol_ = protocol;
  return self;
}
...
- (id<UTCPTChannelDelegate>)delegate {
  return delegate_;
}


- (void)setDelegate:(id<UTCPTChannelDelegate>)delegate {
  delegate_ = delegate;
  delegateFlags_ = 0;
  if (!delegate_) {
    return;
...
- (void)connectToPort:(int)port overUSBHub:(UTCPTUSBHub*)usbHub deviceID:(NSNumber*)deviceID callback:(void(^)(NSError *error))callback {
...
- (void)connectToPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^)(NSError *error, UTCPTAddress *address))callback {
  assert(protocol_ != NULL);
  if (connState_ != kConnStateNone) {
  ...
  // Success
  NSError *err = nil;
  UTCPTAddress *ptAddr = [[UTCPTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
...
- (BOOL)acceptIncomingConnection:(dispatch_fd_t)serverSocketFD {
  ...
  if (delegateFlags_ & kDelegateFlagImplements_ioFrameChannel_didAcceptConnection_fromAddress) {
    UTCPTChannel *peerChannel = [[UTCPTChannel alloc] initWithProtocol:protocol_ delegate:delegate_];
    __block UTCPTChannel *localChannelRef = self;
    ...
    UTCPTAddress *address = [[UTCPTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
...
- (BOOL)startReadingFromConnectedChannel:(dispatch_io_t)channel error:(__autoreleasing NSError**)error {
  ...
  [protocol_ readFramesOverChannel:channel onFrame:^(NSError *error, uint32_t type, uint32_t tag, uint32_t payloadSize, dispatch_block_t resumeReadingFrames) {
    if (handleError(error, type == UTCPTFrameTypeEndOfStream)) {
      return;
    }
    ...
    if (payloadSize == 0) {
      ...
      if (!accepted) {
        ...
        [protocol_ readPayloadOfSize:payloadSize overChannel:channel callback:^(NSError *error, dispatch_data_t contiguousData, const uint8_t *buffer, size_t bufferSize) {
          ...
          if (delegate_) {
            UTCPTData *payload = [[UTCPTData alloc] initWithMappedDispatchData:contiguousData data:(void*)buffer length:bufferSize];
...
- (NSInteger)port {
  if (sockaddr_.ss_len) {
    return ntohs(UTCPT_SOCKADDR_ACCESS(&sockaddr_, sin_port, sin6_port));
  } else {
    return 0;
  }
}
