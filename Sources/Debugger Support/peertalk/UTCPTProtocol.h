...
static const uint32_t UTCPTFrameNoTag = 0;

// Special frame type that signifies that the stream has ended.
static const uint32_t UTCPTFrameTypeEndOfStream = 0;

// NSError domain
FOUNDATION_EXPORT NSString * const UTCPTProtocolErrorDomain;


@interface UTCPTProtocol : NSObject
...
// When the stream ends, a frame of type UTCPTFrameTypeEndOfStream is received.
...
// Read a single frame over *channel*. A frame of type UTCPTFrameTypeEndOfStream
